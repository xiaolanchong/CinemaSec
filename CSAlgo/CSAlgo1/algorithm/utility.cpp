...
/** \brief Function initializes parameter structure from the binary image of parameters.
  ...
  \param  binParams  the binary image of input parameters, empty container causes default settings.
  \param  params     out: reference to the parameter structure to be initialized.
  \return            Ok = S_OK, otherwise E_FAIL. */
//-------------------------------------------------------------------------------------------------
HRESULT LoadParameters( const Int8Arr & binParams, Parameters & parameters )
{
  ...
  try
  {
    ...
    {
      const wchar_t * text = reinterpret_cast<const wchar_t*>( &(*(binParams.begin())) );
      StdStrStream    strm;
      int             length = (int)(binParams.size()/sizeof(wchar_t));

      ALIB_ASSERT( (length > 0) && (text[length-1] == _T('\0')) );
      strm << text;
      ALIB_ASSERT( parameters.load( strm ) );
    }
    retValue = S_OK;
  }
