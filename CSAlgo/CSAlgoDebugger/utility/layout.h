...
#define  MY_ONE_IMAGE_LAYOUT_H

//=================================================================================================
/** \class OneImageLayout.
    \brief Object defines the single-image layout. */
//=================================================================================================
class OneImageLayout
{
...
void p2s( const POINT & point, POINT & screen )
{
  if (m_bReady)
  {
    int y = (ALIB_INVERT_Y) ? point.y : (m_imgH-1-point.y);
...
void p2s( const Vec2f & point, POINT & screen )
{
  if (m_bReady)
  {
    float y = (ALIB_INVERT_Y) ? point.y : (float)(m_imgH-1.0f-point.y);
...
void s2p( const POINT & screen, POINT & point )
{
  if (m_bReady)
  {
    ...
    if (!ALIB_INVERT_Y) point.y = m_imgH-1-point.y;
  }
...
void s2p( const POINT & screen, Vec2f & point )
{
  if (m_bReady)
  {
    ...
    if (!ALIB_INVERT_Y) point.y = (float)(m_imgH-1.0f-point.y);
  }
  else point.x = (point.y = 0);
}
