...
void	XMLDocument::SaveToString( std::tstring& sData ) const
{
	//UNREFERENCED_PARAMETER(sData);
	//BSTR s;
	CComBSTR sb;
	HRESULT hr = m_pDoc->get_xml( &sb );
	//sb.Attach(s);
...
{
#if 0
	return /*m_iXMLList.p == itr.m_iXMLList.p*/
		(const_cast< CComPtr<IXMLDOMNodeList>& >(m_iXMLList)).IsEqualObject( itr.m_iXMLList.p) && m_Position == itr.m_Position;
#else
...
std::wstring XMLDocument::GetParsedError() const
{
	...
	CComBSTR str;
	HRESULT hr = pError->get_reason(&str);
	std::wstring z ( hr == S_OK ? str : L"" );
	hr = pError->get_srcText(&str);
	z += hr == S_OK ? str : L"" ;
	long pos;
...
XMLProcInstr::XMLProcInstr(	const XMLDocument& Doc, const std::tstring & Target, const std::tstring &Data )
{
	CComBSTR bstrTarget(Target.c_str()), bstrData( Data.c_str() );
	HRESULT hr = Doc.m_pDoc->createProcessingInstruction( bstrTarget, bstrData, &m_pNative  );
	HrCheck<XMLProcInstrException>(hr, "Failed to create processing instruction");
}

XMLText::XMLText(const XMLElement& Tree,const std::tstring& Value)
{
	CComBSTR var(Value.c_str());
