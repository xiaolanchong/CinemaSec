...
public:
	...
	XMLText(const XMLElement& Tree,const std::tstring& Value);
};
...
public:	
	...
	{
		CComVariant val( VT_BSTR );
		m_iXMLElem->get_nodeTypedValue(&val);
		USES_CONVERSION;
		LPCTSTR szValue = W2CT(val.bstrVal);
		return std::tstring(szValue);
	}
	...
	std::tstring GetAttr( const std::tstring& Name, std::tstring& Value) 
	{
		CComVariant VarGeneric( VT_EMPTY );
		CComBSTR bstrName( Name.c_str() );
		HRESULT hr =  m_iXMLElem->getAttribute(bstrName, &VarGeneric);
...
//		assert(var.vt == (VT_ARRAY| Type2Var<ElementType>::Type));
...
struct XMLProcInstr
{
	...
	XMLProcInstr(	const XMLDocument& Doc, const std::tstring & Target, const std::tstring &Data );
