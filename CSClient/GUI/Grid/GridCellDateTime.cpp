							 ...
							 UINT nID, UINT nChar)
{
	...
	m_pEditWnd = new CInPlaceDateTime(GetGrid(), rect,
		m_dwStyle|DTS_UPDOWN, m_dwExStyle, nID, nRow, nCol, 
...
void CGridCellDateTime::Init(DWORD dwStyle, DWORD dwExStyle)
{
	Init( CTime::GetCurrentTime(), dwStyle, dwExStyle);
}

void CGridCellDateTime::Init(const CTime& Time, DWORD dwStyle, DWORD dwExStyle )
{
	m_cTime		= Time;
	m_dwStyle	= dwStyle;
	m_dwExStyle = dwExStyle;

	if( dwExStyle & DTS_EX_NO_INIT_VALUE)
	{

	}
	else if( dwExStyle & DTS_EX_SHOW_MINUTES)
	{
		SetText(m_cTime.Format(_T("%H:%M")));
	}
	else if (DTS_TIMEFORMAT == m_dwStyle) 
	{
...
CInPlaceDateTime::CInPlaceDateTime(CWnd* pParent, CRect& rect, DWORD dwStyle, DWORD dwExStyle, UINT nID,
								   ...
								   UINT nFirstChar)
{
	...
	if( dwExStyle & DTS_EX_SHOW_MINUTES)
	{
