...
void CSceneToolSet::SaveScene( const wstring& sFileName, CScene* pSceneComponent )
{
	try
	{
		...
		nodes.push_back( make_pair( pSceneComponent, pXMLElement ) );

		for( NodeElement::iterator it = nodes.begin() ; it != nodes.end(); ++it )
		{
			pParentNode = it -> first;
			pXMLParent = it -> second;
			...
			else if( pCamera = dynamic_cast<CCamera*>(pParentNode) )
			{
				...
				//idx
				Convert::FromType( pCamera->GetIdX(), sAttrVal );
				varAttrVal = sAttrVal.c_str();
				pXMLCamera->setAttribute( L"idx", varAttrVal );

				//idy
				Convert::FromType( pCamera->GetIdY(), sAttrVal );
				varAttrVal = sAttrVal.c_str();
			...
			for( int i = 0 ; pChildNode = pParentNode->GetChild(i); ++i )
				nodes.push_back( make_pair( pChildNode, pXMLElement ) );
		}
